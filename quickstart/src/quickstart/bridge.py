...
def copy_default_bridge_config(user_file, bridge_config_file):
    shutil.copyfile(bridge_config_file, user_file)


def _should_overwrite_prompt(base_dir, bridge_config_file):
    while True:
        choice = click.prompt(
            fill(
...
def _write_bridge_config_dialog(
    *, base_dir, bridge_config_file, user_file, foreign_chain_name
):
    choice = click.prompt(
        fill(
