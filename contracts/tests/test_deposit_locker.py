...
@pytest.fixture()
def deposit_contract_on_longer_chain(chain_cleanup,
...
def test_init_already_initialized(deposit_locker_contract, accounts):
    ...
    with pytest.raises(eth_tester.exceptions.TransactionFailed):
        contract.functions.init(release_block_number, validator_contract_address).transact({"from": accounts[0]})
...
def test_init_not_owner(non_initialised_deposit_locker_contract_session, accounts):
    ...
    with pytest.raises(eth_tester.exceptions.TransactionFailed):
        contract.functions.init(release_block_number, validator_contract_address).transact({"from": accounts[1]})


def test_init_passed_realease_block(non_initialised_deposit_locker_contract_session, accounts, web3):
    contract = non_initialised_deposit_locker_contract_session
    validator_contract_address = accounts[0]
    release_block = web3.eth.blockNumber - 1

    with pytest.raises(eth_tester.exceptions.TransactionFailed):
        contract.functions.init(release_block, validator_contract_address).transact({"from": web3.eth.defaultAccount})


def test_owner_after_init(deposit_locker_contract):
    contract = deposit_locker_contract

    assert contract.functions.owner().call() == "0x0000000000000000000000000000000000000000"
...
def test_deposit(deposit_locker_contract, accounts, web3):
    ...
    deposit = 10000000
    tx = contract.functions.deposit(accounts[2]).transact({"from": accounts[2], "value": deposit})
...
def test_withdraw_too_soon(deposit_locker_contract_with_deposits, accounts, deposit_amount):
...
def test_deposit_not_initialised(non_initialised_deposit_locker_contract_session, accounts, deposit_amount):
    contract = non_initialised_deposit_locker_contract_session

    with pytest.raises(eth_tester.exceptions.TransactionFailed):
        contract.functions.deposit(accounts[0]).transact({"from": accounts[0], "value": deposit_amount})


def test_withdraw_not_initialised(non_initialised_deposit_locker_contract_session, accounts):
    contract = non_initialised_deposit_locker_contract_session
...
def test_slash_not_initialised(non_initialised_deposit_locker_contract_session, accounts):
    contract = non_initialised_deposit_locker_contract_session
...
def test_event_deposit(deposit_locker_contract, accounts, web3):
    ...
    deposit = 10000000
    contract.functions.deposit(accounts[0]).transact({"from": accounts[0], "value": deposit})

    event = contract.events.Deposit.createFilter(
        fromBlock=latest_block_number).get_all_entries()[0]["args"]

    assert event['depositOwner'] == accounts[0]
    assert event['value'] == deposit


def test_event_withdraw(deposit_contract_on_longer_chain, malicious_non_validator_address, web3):
    contract = deposit_contract_on_longer_chain
