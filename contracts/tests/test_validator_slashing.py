...
def test_init_already_initialized(validator_slasher_contract, accounts):
    ...
    with pytest.raises(eth_tester.exceptions.TransactionFailed):
        contract.functions.init([accounts[0]], fund_contract_address).transact({"from": accounts[0]})


def test_init_not_owner(non_initialised_validator_slasher_contract_session, accounts, validators):
    contract = non_initialised_validator_slasher_contract_session
    fund_contract_address = accounts[0]

    with pytest.raises(eth_tester.exceptions.TransactionFailed):
        contract.functions.init(validators, fund_contract_address).transact({"from": accounts[1]})


def test_remove_validator1(validator_slasher_contract, validators, accounts):
    contract = validator_slasher_contract

    contract.functions.testRemoveValidatorFromSet(validators[1]).transact({"from": accounts[0]})
...
def test_remove_validator0(validator_slasher_contract, validators, accounts):
    ...
    contract.functions.testRemoveValidatorFromSet(validators[0]).transact({"from": accounts[0]})
...
def test_report_malicious_validator_malicious_validator(
    ...
    assert (deposit_locker_contract_with_deposits.functions.deposits(malicious_validator_address).call() ==
            deposit_amount)

    assert (validator_slasher_contract.functions.getValidator(2).call() ==
            malicious_validator_address)
    ...
    assert deposit_locker_contract_with_deposits.functions.deposits(malicious_validator_address).call() == 0

    assert (validator_slasher_contract.functions.getValidator(0).call() ==
            validators[0])

    assert (validator_slasher_contract.functions.getValidator(1).call() ==
            validators[1])
