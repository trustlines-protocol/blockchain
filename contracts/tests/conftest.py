...
def validator_set_contract_session(deploy_contract, validators, web3):
    ...
    initialized_test_validator_set = initialize_validator_set(deployed_test_validator_set, validators, web3=web3)
...
def initialised_deposit_and_slasher_contracts(validators, deploy_contract, web3):
    ...
    initialised_slasher_contract = initialize_test_validator_slasher(slasher_contract,
                                                                     validators,
                                                                     fund_contract_address,
                                                                     web3)

    Deposit_slasher_contracts = namedtuple('Deposit_slasher_contracts', 'deposit_contract, slasher_contract')
    return Deposit_slasher_contracts(deposit_contract=initialised_deposit_contract,
...
@pytest.fixture
def deposit_locker_contract_with_deposits(chain_cleanup,
    ...
    for validator in validators:
        deposit_contract.functions.deposit(validator).transact({"from": validator, "value": deposit_amount})
...
def two_signed_blocks_no_list_header(
    ...
    signature_one = malicious_validator_key.sign_msg(rlp_unsigned_block_header_one).to_bytes()
    signature_two = malicious_validator_key.sign_msg(rlp_unsigned_block_header_two).to_bytes()
...
def two_signed_blocks_different_signer(
    ...
    signature_one = malicious_validator_key.sign_msg(rlp_unsigned_block_header_one).to_bytes()
    signature_two = malicious_non_validator_key.sign_msg(rlp_unsigned_block_header_two).to_bytes()
...
def two_signed_blocks_different_height(
    ...
    unsigned_block_header_one = rlp.decode(two_equivocating_block_header[0])
    unsigned_block_header_one[8] = rlp.decode(unsigned_block_header_one[8], rlp.sedes.big_endian_int) + 1
    rlp_unsigned_block_header_one = rlp.encode(unsigned_block_header_one)
    rlp_unsigned_block_header_two = two_equivocating_block_header[1]
